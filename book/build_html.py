...
def enrich_segment(s):
    s=CASE_US.sub(lambda m:A(look("case",f"{m.group(1)} US {m.group(2)}",
        f"https://supreme.justia.com/cases/federal/us/{m.group(1)}/{m.group(2)}/"),"case",m.group(0)),s)
    s=CASE_F.sub(lambda m:A(look("case",m.group(0),
        "https://www.courtlistener.com/?q="+quote(m.group(0))),"case",m.group(0)),s)
    s=USC.sub(lambda m:A(look("usc",m.group(1),
        f"https://www.law.cornell.edu/uscode/text/26/{m.group(1)}"),"stat",m.group(0)),s)
    def cfr_repl(m):
        ...
        return A(look("cfr",key,fb),"reg",m.group(0))
    s=CFR.sub(cfr_repl,s)
    s=AMEND.sub(lambda m:A(look("const",m.group(1)+" Amendment",
        "https://constitution.congress.gov/constitution/"+AMAP[m.group(1).lower()]+"/"),"const",m.group(0)),s)
    s=TD.sub(lambda m:A(look("td",m.group(1),
        td_map.get(m.group(1),f"https://babel.hathitrust.org/cgi/ls?q1=%22Treasury+Decision+{m.group(1)}%22;a=srchls;lmt=ft")),
        "td",m.group(0),f' data-td="{m.group(1)}"'),s)
    s=FORM.sub(lambda m:A(look("form","Form "+m.group(1),
        "https://www.irs.gov/forms-pubs/about-form-"+m.group(1).strip().lower().replace(" ","-")),"form",m.group(0)),s)
    return s
